...
if __name__ == "__main__":
    ...
    # TODO: if we ever go above 100 repos, we need to paginate
    ...
    for rock in rocks_projects:
        ...
        # url = f"{GIT_API_URL}/repos/{ORG}/{rock['name']}/branches"
        url = f"{GIT_API_URL}/repos/{ORG}/{rock['name']}/git/matching-refs/heads/channels/"
        rock_branches = get_all_pages(url, headers, {})
        # rock_branches = list(filter(lambda b: b["name"].startswith("channels/"), branches))
    ...
    url = f"{GIT_API_URL}/repos/{os.getenv('GITHUB_REPOSITORY', f'{ORG}/.builder')}/actions/runs"
    params = {
        "branch": "main",
        "status": "in_progress",
